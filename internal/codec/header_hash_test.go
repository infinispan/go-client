...
func TestReadResponseHeaderWithHashTopology(t *testing.T) {
	...
	// topology update
	WriteVInt(&buf, 10)            // topology id
	WriteVInt(&buf, 3)             // 3 servers
	WriteLPString(&buf, "node-a")  // host 0
	WriteU2(&buf, 11222)           // port 0
	WriteLPString(&buf, "node-b")  // host 1
	WriteU2(&buf, 11222)           // port 1
	WriteLPString(&buf, "node-c")  // host 2
	WriteU2(&buf, 11222)           // port 2

	// hash distribution data (only read when clientIntelligence == HASH_DIST_AWARE)
	buf.WriteByte(3)   // hash function version
	WriteVInt(&buf, 4) // 4 segments
...
func TestReadResponseHeaderTopologyAwareSkipsHash(t *testing.T) {
	...
	WriteVInt(&buf, 1)                 // 1 server
	WriteLPString(&buf, "server1")     // host
	WriteU2(&buf, 11222)               // port
